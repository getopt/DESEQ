...
#                    --treatCountCols=<1-based column numbers (after rownames), csv>
#                    --contrCountCols=<1-based column numbers (after rownames), csv>
#                    --outfiBaseTAB=<path/nameBase>
#                    --outfiBasePDF=<path/nameBase> 
#                    --filterNoPolya                                                      < thisScript

# Description:
#   --table           <   file with first column as row names and two columns providing counts (values will be rounded to integers)
#   --header          <   "YES" or "NO" (default "NO"; header isn't important as such, only important for correct reading of the table)
...
#   --method          <   "pooled" or "blind" method of fitting dispersion model. Use "pooled" if you have replicates, "blind" if you do not have replicates

#   --sharingMode     <   "maximum" or "fit-only" mode of fitting dispersion model. Use "maximum" if you have replicates, "fit-only" if you do not have replicates

#   --fitType         <   "parametric" or "local" mode of fitting dispersion model.
#                         "parametric" (default) is recommended. But it doesn't work sometimes for unknown reasons, suggested alternative "local" 

#   --outfiBaseTAB    <   path/nameBase of for the output files. Two will be written 
#                                                                           ${base}.deseq.Results_all.tab
#   --outfiBasePDF    <   path/nameBase of for output PDFs. Two will be written: 
#                                                                           ${base}.deseq.dispersion.pdf 
...
if(!is.null(args[["outfiBasePDF"]]))
    outfiBasePDF      <-   args$outfiBasePDF
...
if (is.null(table.file) | is.null(treatCountCols) |  is.null(contrCountCols))
    stop("must have --table=<table.file> and --treatCountCols=<1-based column numbers, csv> options and --contrCountCols=<1-based column numbers, csv> options" )
...
designTable     <-    data.frame(row.names = colnames(countTable), 
                                 condition = c(rep("treated",    times = length(treatCountTable[1,])),
...
## cds   <- estimateDispersions(cds, method="per-condition", sharingMode="maximum" , fitType = fitType) ## this has to be tested

outfile  <-  paste(outfiBasePDF, ".deseq.dispersion.png", sep = "")
mainT    <-  sub("..*\\/","", outfile)
 png(outfile, height = 800, width = 800)
    plot(rowMeans(counts(cds, normalized=TRUE)), fitInfo(cds)$perGeneDispEsts, pch = '.', log="xy", main = mainT)
...
# N B I N O M  T E S T
res    <- nbinomTest(cds, "untreated", "treated") # THIS IS TESTED CORRECT DIRECTIONALITY WHEN DATA ORIGINALLY SUPPLIED IN "TREATMENT,CONTROL" DIRECTION
                                                  # THE RESULTING POSITIVE FOLD CHANGE MEANS UPREGULATION IN TREATMENT
                                                  #               NEGATVIE                   DOWNREGULATION IN TREATMENT
outfile  <-  paste(outfiBasePDF, ".deseq.diffexpres.png", sep = "")
mainT    <-  sub("..*\\/","", outfile)
 png(outfile, height = 800, width =  800)
 ...
 points(res[res$padj < padjCutoff, "baseMean"], res[res$padj < padjCutoff, "log2FoldChange"], col = "red", pch = 19, cex = 1)
 dev.off()
